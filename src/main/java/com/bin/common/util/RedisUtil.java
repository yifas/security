...
public class RedisUtil {
     ...
     * @return 所有值
     */
    public Set<Object> smGet(String key) {
        return redisTemplate.opsForSet().members(key);
    }
     ...
     * @return HV
     */
    public Object getTokenRefreshTime(String key) {
        return hget(key, "refreshTime");
    }

    public void setTokenRefresh(String token) {
        //过期时间 2 天
        Integer expire = 2 * 24 * 60 * 60;
        hset(TOKEN_PREFIX + token, "refreshTime", DateUtil.getAddDaySecond(refreshTime), expire);
        hset(TOKEN_PREFIX + token, "expireTime", DateUtil.getAddDayTime(expireTime), expire);
    }
}
